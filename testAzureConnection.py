...
#     try:
cursor.execute("""
CREATE TABLE households (HSHD_NUM int, L varchar(255), AGE_RANGE varchar(255),MARITAL varchar(255),
INCOME_RANGE varchar(255),HOMEOWNER varchar(255),HSHD_COMPOSITION varchar(255),HH_SIZE varchar(255),
CHILDREN varchar(255),PRIMARY KEY (HSHD_NUM));
        """);
cursor.execute("""
CREATE TABLE products (PRODUCT_NUM int,DEPARTMENT varchar(255),COMMODITY varchar(255),BRAND_TY varchar(255),
NATURAL_ORGANIC_FLAG varchar(255));""");
cursor.execute("""
CREATE TABLE transactions (BASKET_NUM int,HSHD_NUM int,PURCHASE_ varchar(255),PRODUCT_NUM int,SPEND DECIMAL,
UNITS int,STORE_R varchar(255),WEEK_NUM int,YEAR_NUM int);""");
        # cursor.execute(
